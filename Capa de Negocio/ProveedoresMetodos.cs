...
using System.Text;
using System.Data;
using System.Data.SqlClient;//Ultimo erika
using Capa_de_Datos;
using Capa_Entidad;
...
{
    public class ProveedoresMetodos : Coneccion
    {
        ...
        {
            var sqlStr = "select pro_CUIT, pro_RazonSocial, pro_Telefono, pro_Email , dir_Calle + ' '+ CONVERT(varchar(10), dir_Numero )+ ', '+ dir_Localidad + ', '+ dir_Provincia Domicilio, pro_FechaAlta ,pro_FechaBaja from Proveedores , Direcciones where pro_IdDireccion = dir_IdDir and pro_RazonSocial like '%" + RazoSocial + "%' ;";
            var da = new SqlDataAdapter(sqlStr, conectar());
            var ds = new DataSet();
            da.Fill(ds);
            DataTable dt = ds.Tables[0];
